...
export default function CreateEventPage() {
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    try {
      await createEvent(data);
      setShowSuccessMessage(true); // Show success message on successful creation
      // router.push("/events"); // Removed to display message on current page
      // form.reset(); // Removed to display message on current page
      // router.refresh(); // Removed to display message on current page
    } catch (error) {
      console.error("Error creating event:", error);
      form.setError("root", {
        message: "Failed to create event. Please try again.",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="px-4 py-8 mx-auto sm:px-6 lg:px-8 lg:py-12">
        ...
        {showSuccessMessage ? (
          <Card className="mb-8 border-green-200 bg-green-50/80 backdrop-blur-sm dark:border-green-800 dark:bg-green-900/20">
            <CardContent className="pt-6 text-center">
              <div className="flex flex-col items-center justify-center gap-4 text-green-700 dark:text-green-400">
                <CheckCircle className="w-12 h-12" />
                <h2 className="text-3xl font-bold">
                  Event Created Successfully!
                </h2>
                <p className="max-w-md text-lg">
                  Your event has been created successfully but it must go
                  through StartUpSl admin team for approval, rejection or
                  suspend.
                </p>

...
export default function CompanyEditForm({ company }: CompanyEditFormProps) {
  ...
  const [fundingDocFile, setFundingDocFile] = useState<string | null>(null);
  ...
  // Handle logo upload
  const handleLogoChange = (file: string) => {
    setLogoFile(file);
  };

  const handleFundingDocChange = (file: string) => {
  ...
  const handlePitchDeckChange = (url: string) => {
    // if (e.target.files && e.target.files[0]) {
    //   const file = e.target.files[0];
  ...
  const onSubmit = async (data: z.infer<typeof companyFormSchema>) => {
    try {
      ...
      const completeData = {
        ...
        businessChallenges,
        logo: logoFile,
        foundingDocuments: pitchDeckFile,
        pitchDeck: fundingDocFile,
      };
                        ...
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <div className="flex flex-col items-center">
                            <Uploader
                              ...
                              maxSizeMB={10}
                              uploadTitle={
                                "Upload Company Logo (max 2MB) 400x400px"
                              }
                              className="p-6 border-2 border-gray-300 border-dashed rounded-lg"
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <Uploader
                          ...
                          uploadTitle={
                            company?.fundingDocuments
                              ? "Replace funding documents (PDF, DOC, or DOCX up to 10MB)"
                              : "Upload funding documents (PDF, DOC, or DOCX up to 10MB)"
                          }
                          className="p-6 border-2 border-gray-300 border-dashed rounded-lg"
                          onUploadComplete={(urls: any) =>
                      ...
                      <div className="space-y-4">
                        ...
                        <Uploader
                          ...
                          uploadTitle={
                            company?.pitchDeck
                              ? "Replace pitch deck (Recommended PDF, PPT, or PPTX up to 20MB)"
                              : "Upload pitch deck (Recommended PDF, PPT, or PPTX up to 20MB)"
                          }
                          className="p-6 border-2 border-gray-300 border-dashed rounded-lg"
                          onUploadComplete={(urls: any) =>
                            handlePitchDeckChange(urls)
                          }
                        />
                      </div>
                    </div>
